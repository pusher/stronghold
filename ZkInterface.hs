...
data ZkInterface = ZkInterface String (ByteString -> IO ByteString) !(TVar (Maybe ByteString)) (MVar Zoo.ZHandle)

tryGetHeadSTM :: ZkInterface -> STM (Maybe ByteString)
tryGetHeadSTM (ZkInterface _ _ head _) = readTVar head
...
getHead (ZkInterface _ _ head _) = MaybeT (readTVarIO head)
...
fetchHeadAndWatch (ZkInterface _ _ head zk) = do
  zk' <- readMVar zk
...
reconnect interface@(ZkInterface hostPort _ _ zk) = do
  takeMVar zk
...
newZkInterface hostPort = do
  ...
  interface <- ZkInterface hostPort fetchNode' <$> newTVarIO Nothing <*> return zk'
  Zoo.setWatcher zk (Just (watcher interface))
  return interface
 where
...
loadData (ZkInterface _ fetchNode _ zk) ref =
...
storeData (ZkInterface _ _ _ zk) d = do
  zk' <- lift $ readMVar zk
...
hasReference (ZkInterface _ _ _ zk) ref = do
  zk' <- lift $ readMVar zk
...
updateHead (ZkInterface _ _ _ zk) old new = do
  zk' <- lift $ readMVar zk
